...
void Preprocess::mid360_handler(
  ...
  for (int i = 0; i < plsize; i++) {
    // if((pl_orig.points[i].line < num_scans_) && ((pl_orig.points[i].tag &
    // 0x30) == 0x10 || (pl_orig.points[i].tag & 0x30) == 0x00)){
    PointType added_pt;
    ...
    added_pt.curvature =
        float(pl_orig.points[i].timestamp * 1e-9 - msg_header_time) *
        1e3;  // curvature unit: ms
    // std::cout<<"pl_orig.points[i].timestamp: "<<added_pt.curvature<<"
    // "<<pl_orig.points[i].timestamp<<std::endl;
...
void Preprocess::velodyne_handler(
  ...
  if (feature_enabled) {
    ...
    for (int i = 0; i < plsize; i++) {
      ...
      added_pt.z = pl_orig.points[i].z;
      added_pt.intensity = pl_orig.points[i].intensity;
      if (pl_orig.points[i].time < 0.0) {
        added_pt.curvature = (0.1 + pl_orig.points[i].time) * 1e3;
      } else if (pl_orig.points[i].time < 1.0) {
        added_pt.curvature = pl_orig.points[i].time * 1e3;
      } else {
        added_pt.curvature =
            (pl_orig.points[i].time - pl_orig.points[0].time) * 1e-3;
      }
