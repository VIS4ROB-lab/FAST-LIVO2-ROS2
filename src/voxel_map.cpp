...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat) {
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_;
    ...
    for (int i = 0; i < effct_feat_num_; i++) {
      ...
      double sigma_l =
          0;             // J_nq * ptpl_list_[i].plane_var_ * J_nq.transpose();
      double var_l = 0;  //         ptpl_list_[i].normal_.transpose() * var *
                         //         ptpl_list_[i].normal_;

      R_inv(i) = 1.0 / (0.001 + sigma_l + var_l);
      // std::cerr << "sigma_l: " << sigma_l << ", var_l: " << var_l
      // << ", R_inv: " << R_inv(i) << std::endl;
...
#pragma omp parallel for
  ...
  float max_dis_to_plane = 0, min_dis_to_plane = 100, ave_dis_to_plane = 0;
  for (size_t i = 0; i < useful_ptpl.size(); i++) {
    if (useful_ptpl[i]) {
      ptpl_list.push_back(all_ptpl_list[i]);
      max_dis_to_plane =
          fmax(max_dis_to_plane, fabs(all_ptpl_list[i].dis_to_plane_));
      min_dis_to_plane =
          fmin(min_dis_to_plane, fabs(all_ptpl_list[i].dis_to_plane_));
      ave_dis_to_plane += fabs(all_ptpl_list[i].dis_to_plane_);
    }
  }
  ave_dis_to_plane /= ptpl_list.size();
  std::cout << "[ LIO ] min: " << min_dis_to_plane
            << " max: " << max_dis_to_plane << " ave: " << ave_dis_to_plane
            << std::endl;
}
