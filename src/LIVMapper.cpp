...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &node) {
  ...
  try_declare.template operator()<std::string>("common.img_topic",
                                               "/left_camera/image");
  try_declare.template operator()<bool>("common.enable_image_processing",
                                        false);
  ...
  this->node->get_parameter("common.enable_image_processing",
    ...
    for (const auto &dir_path : target_dirs) {
      ...
      if (fs::exists(dir)) {
        try {
          ...
          std::cout << "Removed " << removed_count << " items from " << dir_path
                    << std::endl;
        } catch (const fs::filesystem_error &e) {
          // Handle potential permission errors during deletion
          std::cerr << "Failed to remove " << dir_path << ": " << e.what()
                    << std::endl;
          continue;
        }
      }
      ...
      try {
        ...
        std::cerr << "Failed to create " << dir_path << ": " << e.what()
                  << std::endl;
      }
    }
  }
...
void LIVMapper::initializeSubscribersAndPublishers(
  ...
  if (enable_image_processing) {
    sub_img_compressed =
        this->node->create_subscription<sensor_msgs::msg::CompressedImage>(
            img_topic, 200000,
            std::bind(&LIVMapper::jpeg_callback, this, std::placeholders::_1));
  } else {
...
void LIVMapper::jpeg_callback(
  ...
  try {
    ...
    if (msg_header_time - last_timestamp_img < 0.02) {
      RCLCPP_WARN(node->get_logger(), "Image need Jumps: %.6f",
                  msg_header_time);
      return;
    }
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in) {
  ...
  // RCLCPP_INFO(this->node->get_logger(), "get imu at time: %.6f",
  // stamp2Sec(msg_in->header.stamp));
  ...
  if (last_timestamp_imu > 0.0 && timestamp > last_timestamp_imu + 0.2) {
    RCLCPP_WARN(this->node->get_logger(),
                "imu time stamp Jumps %0.4lf seconds \n",
    ...
    case LIVO: {
      ...
      switch (last_lio_vio_flg) {
        ...
        case LIO: {
          ...
          sig_buffer.notify_all();
          meas.measures.push_back(m);
          lidar_pushed = false;  // after VIO update, the _lidar_frame_end_time
                                 // will be refresh.
...
void LIVMapper::publish_frame_world(
  ...
  laserCloudmsg.header.stamp =
      this->node->get_clock()->now();  //.fromSec(last_timestamp_lidar);
